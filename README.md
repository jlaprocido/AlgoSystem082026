...
- `python -m src.trading.run_daily` — runs once per trading day at 9:45am America/Chicago. Order of operations: is the kill switch already tripped? → is the market even open? → has max drawdown breached? → has today's loss limit breached? → compute today's signal from fresh bars → size and submit a marketable-limit rebalance order if the signal implies a different position, otherwise just hold. **Always sends a notification** either way — a "BOUGHT/SOLD" message on an actual trade, or a daily check-in message ("signal long, holding N shares, no trade needed") when nothing changed, so silence never has to be interpreted as "did this even run today?"
- `python -m src.trading.eod_summary` — runs once after close, at 4:00pm America/Chicago: sends a notification with today's equity, today's PnL, **total PnL since the strategy's first trade** (not since the account was funded — see "Dashboard" below for why that distinction matters), drawdown, and position size.
- `python -m src.trading.intraday_check` — runs hourly, 10am-3pm America/Chicago. Shares `run_daily.py`'s `check_risk_limits()` (same drawdown/daily-loss checks, same flatten-and-log-and-notify behavior) but never computes a signal or opens a new position — it exists purely as a faster circuit breaker. See "Leverage" below for why this exists.
...
**Notifications** (`src/notifications/notifier.py`, optional): a push via [ntfy.sh](https://ntfy.sh) (a plain HTTPS POST to a topic you pick, read via the ntfy app) fires on every `run_daily` run (always, trade or not — see above), on each of the four risk conditions, and on the end-of-day summary. `intraday_check` is the one exception that stays silent on a clean check — it runs hourly, so notifying on every "all fine" would be pure noise; it still speaks up immediately on an actual breach. Started as email-to-SMS (Gmail SMTP → a carrier's gateway address), but two different gateways (AT&T's `txt.att.net`, then a third-party alternative) both silently failed to deliver — email relay failures happen *asynchronously* (a bounce email arrives back at the sender, sometimes minutes later), so a script has no way to detect them at send time at all. ntfy's plain synchronous HTTP response fixes that: a delivery failure raises immediately, in the same process, instead of arriving as an email bounce after the script has already exited. A notification failure is always non-fatal either way — it never blocks the actual trading/risk logic (see `run_daily.py`'s `notify()` wrapper).
...
`src/dashboard/build_dashboard.py` renders a single static `index.html` (dark theme, no JavaScript, no external assets — everything, including the equity curve, is inline SVG/CSS) from `data/orders/trade_log.csv` plus Alpaca's live account/portfolio-history endpoints: current equity, today's PnL, **total PnL**, drawdown from peak, annualized Sharpe ratio, position size, a halted-status banner when relevant, an equity chart (with auto-scaled `$`/date axes) comparing the strategy against a live AAPL buy-and-hold benchmark, and a recent-activity table. No database — the CSV and Alpaca's own API are the only sources of truth, matching the rest of this system's design.

**The chart/Total-PnL/Sharpe window starts at the strategy's first trade, not the account's funding date** — `executor.find_strategy_start_date()` reads the trade log for the earliest `filled` order and uses that as the boundary, trimming `risk_manager.get_real_equity_history()`'s output (which itself already strips Alpaca's own padding: every day before the account had *any* activity comes back as a literal `0.0`, not `None`) to start there. This matters because an account can sit funded-but-flat for weeks before the strategy's first real trade — counting that idle stretch as part of "the strategy's performance" would understate a real edge (all those zero-return days dilute Sharpe) and make the buy-and-hold comparison meaningless (the benchmark would show whatever AAPL did while the bot was doing literally nothing). The window is fully dynamic — it's whatever the trade log says today, not a hardcoded range — and both series (strategy and benchmark) always use the identical window so they stay directly comparable.
