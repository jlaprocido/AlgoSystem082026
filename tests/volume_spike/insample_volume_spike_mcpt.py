...
from tests.volume_spike.volume_spike import optimize_volume_spike_strategy, INTERVAL
...
for perm_i in tqdm(range(1, n_permutations)):
    # get_permutation shuffles volume alongside the intrabar price shuffle, so a bar's
    # volume stays paired with its own range -- required since this strategy reads volume
    train_perm = get_permutation(train_df)
    _, best_perm_pf = optimize_volume_spike_strategy(train_perm, interval=INTERVAL)  # type: ignore[reportArgumentType]
